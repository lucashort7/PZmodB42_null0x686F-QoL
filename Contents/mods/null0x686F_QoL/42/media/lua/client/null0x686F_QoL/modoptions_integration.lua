...
local function _init_mod_options()
  ...
  local def_enabled = qol_setup.is_zombie_outline_enabled()
  ...
  options:addTickBox("QoL_InvName", "Player Name in Inventory", qol_setup.is_feature_enabled("inv_name"), "Adds the character's name to the inventory window title.")
  options:addTickBox("QoL_HideWorn", "Hide Worn Items", qol_setup.is_feature_enabled("hide_worn"), "Hides worn clothing from the main inventory list to declutter.")
  options:addTickBox("QoL_QuickContext", "Quick Context Menu Actions", qol_setup.is_feature_enabled("quick_context"), "Adds 1-click context menu shortcuts for Rip All Clothing, Swap Gas Mask Filter, and Bandages.")
  options:addTickBox("QoL_WalkAndEquip", "Walk/Aim While Equipping Clothing", qol_setup.is_feature_enabled("walk_and_equip"), "Allows walking or aiming while putting on/adjusting clothing without interrupting the action.")

  options:addTitle(" ")
  -- Survival & World Interactions
  options:addTitle("--- Survival & World Interactions ---")
  options:addTickBox("QoL_RackDry", "Rack Firearm to Dry", qol_setup.is_feature_enabled("rack_dry"), "Press Rack Firearm key to dry yourself when wet and empty-handed.")
  options:addTickBox("QoL_AutoUnsetAlarms", "Auto-Unset Alarms", qol_setup.is_feature_enabled("auto_unset_alarms"), "Automatically turns off alarms on looted digital watches and clocks.")
  options:addTickBox("QoL_FencePriority", "Fence Jump Priority", qol_setup.is_feature_enabled("fence_priority"), "Prioritizes vaulting fences over ground item interactions when close to fences.")
  options:addTickBox("QoL_Vehicles", "Vehicle Gas QoL", qol_setup.is_feature_enabled("vehicle_gas"), "Allows aiming while pumping gas and restores original equipped items when done.")

  options:addTitle(" ")
  -- Weapons
  options:addTitle("--- Weapons ---")
  options:addTickBox("QoL_AutoEquipBroken", "Auto-Handle Broken Weapons", qol_setup.is_feature_enabled("auto_equip_broken_weapon"), "Automatically handles your weapon when it breaks, and lets you re-equip a fresh copy with a hotkey.")
...
qol_setup.sync_mod_options = function ()
  ...
  if opt_enabled then
    ...
    qol_setup.set_zombie_outline_enabled(is_enabled)
  end
  ...
  for opt_key, feature_key in pairs(feature_map) do
    ...
    if opt_obj then
      ...
      qol_setup.set_feature_enabled(feature_key, val)
    end
  end
