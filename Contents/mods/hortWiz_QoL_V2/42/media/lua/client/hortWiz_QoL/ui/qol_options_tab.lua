...
function QoLOptionsTabUI:initialise()
  ...
  self.title_lbl = ISLabel:new(12, 10, 20, "HortWiz QoL V2 - Live Feature Control", 1, 1, 1, 1, font, true)
...
function QoLOptionsTabUI:on_tick_toggled(index, selected)
  ...
  if PZAPI and PZAPI.ModOptions and PZAPI.ModOptions.getOptions then
    local opts = PZAPI.ModOptions:getOptions("hortWiz_QoL")
    if opts then
...
local function _register_qol_tab()
  if _G.HortWizCore and _G.HortWizCore.registerTab then
    _G.HortWizCore.registerTab("QoL Options", QoLOptionsTabUI, "HORTWIZ")
  end
end
